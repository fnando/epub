...
module Epub
  class Epub
    ...
    #
    # Notice that JDCN (Japan Digital Comic Number) will be detected using the
    # following regular expression: `^\d+DD$`.
    #
    ...
    # The navigation that will be used to generate the table of contents.
    #
    # The navigation must look like this:
    #
    #   [{title: "Item 1", target: "item1.html#anchor", navigation: []}]
    #
    # Notice that each navigation entry can have a nested navigation, allowing
    # for a deep hierarchy.
    #
    ...
    def initialize(**kwargs)
      ...
      self.id ||= SecureRandom.uuidv4
      self.epub_engine ||= V3
      self.language ||= "en"
      self.tmpdir ||= File.join(Dir.tmpdir, SecureRandom.uuid)
      self.tmpdir = Pathname.new(tmpdir)
      self.root_dir = Pathname.new(root_dir || Dir.pwd)
      self.files = files.map {|file| Pathname.new(file) }
